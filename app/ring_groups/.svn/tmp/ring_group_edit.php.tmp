...
//get http post variables and set them to php variables
	if (count($_POST) > 0) {
		//set variables from http values
			...
			$ring_group_record = check_str($_POST["ring_group_record"]);
...
//pre-populate the form
	...
	if (strlen($ring_group_uuid) > 0) {
		...
		foreach ($result as &$row) {
			...
			$ring_group_record = $row["ring_group_record"];
		}
...
//show the content
	...
	echo "	<select class='formfld' name='ring_group_record'>\n";

	if (! $ring_group_record) {
		echo "	<option value='0' selected='selected'>". ' No Record '."</option>\n";	
	}
	else {
		echo "	<option value='0' selected='selected'>". ' No Record '."</option>\n";
	}
	
	if ($ring_group_record == "1") {
		echo "	<option value='1' selected='selected'>". ' Record ALL '."</option>\n";
	}
	else {
		echo "	<option value='1'>". ' Record ALL ' ."</option>\n";
	}
	
	if ($ring_group_record == "2") {
		echo "	<option value='2' selected='selected'>". ' Record Call selectively by pressing a key sequence ' ."</option>\n";
	}
	else {
		echo "	<option value='2'>".' Record Call selectively by pressing a key sequence '."</option>\n";
	}
