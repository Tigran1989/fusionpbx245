...
//get the http values and set them as php variables
	if (count($_POST) > 0) {
		//get the values from the HTTP POST and save them as PHP variables
			...
			$dynamic_callerid = check_str($_POST["dynamic_callerid"]);			
			...
			$auto_forward = check_str($_POST["auto_forward"]);
		...
		//update the database
			if ($action == "update" && permission_exists('extension_edit')) {
				...
				//update extensions
					...
					$sql .= "dynamic_callerid = '$dynamic_callerid', ";
...
//				echo "				<a href='extension_edit.php?id=".$extension_uuid."&domain_uuid=".$_SESSION['domain_uuid']."&user_uuid=".$field['user_uuid']."&a=delete' alt='".$text['button-delete']."' onclick=\"return confirm('".$text['confirm-delete']."')\">$v_link_label_delete</a>\n";
	...
	echo "<tr>\n";
	echo "<td class='vncell' valign='top' align='left' nowrap='nowrap'>\n";
	echo "    Dynamic Callerid\n";
	echo "</td>\n";
	echo "<td class='vtable' align='left'>\n";
	echo "    <select class='formfld' name='dynamic_callerid'>\n";
	
	if ($dynamic_callerid == "false") {
		echo "    <option value='false' selected >".$text['label-false']."</option>\n";
	}
	else {
		echo "    <option value='false'>".$text['label-false']."</option>\n";
	}
	if ($dynamic_callerid == "true") {
	...
	echo "<br />\n";
	echo "<br />\n";
	echo "Enable / Disable dynamic callerid\n";
	echo "</td>\n";
	echo "</tr>\n";
...
//				echo "				<a href='extension_edit.php?id=".$extension_uuid."&device_line_uuid=".$row['device_line_uuid']."&domain_uuid=".$_SESSION['domain_uuid']."&a=delete' alt='".$text['button-delete']."' onclick=\"return confirm('".$text['confirm-delete']."')\">$v_link_label_delete</a>\n";
	...
	echo "</td>\n";
	echo "<td class='vtable' align='left'>\n";
	echo "    <select class='formfld' name='auto_forward'>\n";
	
	if ($auto_forward == "false") {
		echo "    <option value='false' selected='selected'>".$text['label-false']."</option>\n";
	}
	else {
		echo "    <option value='false'>".$text['label-false']."</option>\n";
	...
	echo "    </select>\n";
	echo "<br />\n";
	echo "Incoming Calls called by this extension last time will be auto answered by this extension \n";
	echo "</td>\n";
	echo "</tr>\n";
