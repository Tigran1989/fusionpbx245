	...
	function process_xml_cdr($db, $leg, $xml_string) {
		...
		//get the values from the callflow.
			...
			foreach ($xml->callflow as $row) {
				...
				#set the destination number of  ringgroup application
                                if (urldecode($xml->variables->last_sent_callee_id_number) && urldecode($xml->variables->ring_group_uuid)) {
                                        $database->fields['destination_number'] = urldecode($xml->variables->last_sent_callee_id_number);
                                }

				$x++;
			}
			if ($xml->variables->iscallback) {
                                $database->fields['caller_id_name'] = 'callback';
                                $database->fields['caller_id_number'] = $xml->variables->iscallback;
                        }
			unset($x);
		...
		//store post dial delay, in milliseconds
			...
			$database->fields['from_did'] = check_str(urldecode($xml->variables->sip_req_user));
...
//							print_r($db2->result);
							...
							break;
					case "inbound":
							$sql_user_rate = "SELECT v_lcr.rate, v_lcr.connect_increment, v_lcr.talk_increment, v_lcr.currency FROM v_lcr WHERE v_lcr.carrier_uuid IS NULL AND v_lcr.enabled='true' AND v_lcr.lcr_direction='inbound' AND v_lcr.digits IN (".number_series($n).") ORDER BY digits DESC, rate ASC, date_start DESC LIMIT 1";
							if ($debug) {
								echo "sql_user_rate: $sql_user_rate\n";
							}
							...
							break;
					case "local":
							$sql_user_rate = "SELECT v_lcr.rate, v_lcr.connect_increment, v_lcr.talk_increment, v_lcr.currency FROM v_lcr WHERE v_lcr.carrier_uuid IS NULL AND v_lcr.enabled='true' AND v_lcr.lcr_direction='local' AND v_lcr.digits IN (".number_series($n).") ORDER BY digits DESC, rate ASC, date_start DESC LIMIT 1";
							if ($debug) {
								echo "sql_user_rate: $sql_user_rate\n";
							}
	...
	if (strlen($_POST["cdr"]) > 0) {
		...
		//check for the correct username and password
			if ($_SESSION["xml_cdr"]["username"] == $_SERVER["PHP_AUTH_USER"] && $_SESSION["xml_cdr"]["password"] == $_SERVER["PHP_AUTH_PW"]) {
				...
				echo  $_SERVER["PHP_AUTH_USER"], ": ",  $_SERVER["PHP_AUTH_PW"] . "access denied<br />\n";
				return;
			}
