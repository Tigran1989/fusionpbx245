...
//set 24hr or 12hr clock
	...
	foreach($result as $row) {
		if ($row['extension_uuid']) {
			$extension_uuid_list .= $extension_uuid_list ? ',' : '';
			$extension_uuid_list .= "'" . $row['extension_uuid'] . "'";
		}
	}
	
	unset($sql, $prep_statement, $result);
	
	if ($extension_uuid_list) {
	$sql = "select extension from v_extensions  where  domain_uuid='$domain_uuid' and extension_uuid in ($extension_uuid_list)";
...
//get the results from the db
	...
	if (!if_group("superadmin") && !if_group("admin")) {
		$sql .= " and (caller_id_number in ($extension_list) or destination_number in ($extension_list)) ";
	}
